...
class RssCategory():
	def rss_cat_data(self):
		#311, 1274, 1282,1270, 20, 15,1283,1265,1279,1256,1261,1284,1266,1280,1262,1271,1276,1267,1258,1272,1281,1263,1277,1268,1250
		...
		data=Postgresql().get_all("""SELECT * from public.rss_feed_cat where cat_id in (1251,22,907,13,18,5,2, 786,1000,6,10,17,785,1071,908,19,24,1247,1252,1257,1248,1253);""")
		# print(data)
		return data
...
class  CreateTable():
	def RssCreate(self):
		try:
			...
			info = requests.get('http://192.168.0.145:8000/api/rss_data/?cat_id=1250', headers=headers).json()
			...
			for each in info['response']:
				...
				try:
					...
					values('{}','{}','{}','{}','{}',1250)""".format(each['company'],each['experience'],each['salary'],each['location'],each['qualification']))
				except:
					raise
			# Postgresql().execute_query("""CREATE TABLE IF NOT EXISTS public.rss_data (id bigserial NOT NULL,company varchar(200),experience varchar(200),salary varchar(200),location varchar(200),qualification varchar(200),cat_id integer,loan boolean default false,PRIMARY KEY (id))""")
			print('donee')
